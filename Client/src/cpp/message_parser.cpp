#include "../hpp/message_parser.hpp"
...
message_parser::parsed_message message_parser::parse(chat_message const &msg)
{
    ...
    switch (res.type)
    {
        ...
        {
            auto [idargs, strargs] = extract_args(msg, 1, 4, 1);
        ...
        {
            auto [idargs, strargs] = extract_args(msg, 1, 2, 1);
        ...
        case message_types::LOAD_PARTICIPANTS:
        {
            ...
            auto [idargs, strargs] = extract_args(msg, 1 + sizeof(uint64_t), quantity + 1, quantity);
        ...
        case message_types::SEARCH_USERS:
        {
            ...
            auto [idargs, strargs] = extract_args(msg, 1 + sizeof(uint64_t), quantity, quantity);
        ...
        {
            auto [idargs, strargs] = extract_args(msg, 1, 3, 2);
        ...
        break;
        case message_types::END_LOADING: break;
        default:
        {
...
std::pair<message_parser::idsvec, message_parser::strvec> message_parser::extract_args(chat_message const &msg, size_t start, int idc, int strc)
{
    return std::make_pair(extract_ids(msg, start, idc),
                          extract_strs(msg, start + idc * 8, strc));
}

message_parser::idsvec message_parser::extract_ids(chat_message const &msg, size_t start, int idc)
{
    idsvec res(idc);
...
message_parser::strvec message_parser::extract_strs(chat_message const &msg, size_t start, int strc)
{
    strvec res;
    res.reserve(strc);
